...
if pkg_version.is_available("vllm"):
    if pkg_version.is_version_greater_or_equal("vllm", "0.8.5"):
        VLLM_TOKEN_OUTPUT_IDENTIFIER = "output_ids"
    else:
...
class RemoteVLLMEngine(InferenceEngine):
    ...
    async def agenerate(self, req: LLMRequest) -> LLMResponse:
        ...
        # vLLM 的 stop_token_ids 需放在 extra_body
        payload: Dict[str, Any] = {
            "prompt": req.input_ids,               # 直接把 token list 传给 vLLM
            "max_tokens": gconfig.max_new_tokens,
            "temperature": 0.0 if gconfig.greedy else gconfig.temperature,
            "top_p": gconfig.top_p,
            "top_k": gconfig.top_k,
            "stop_token_ids": getattr(gconfig, "stop_token_ids", []),
            "logprobs": 1,                         # 返回每条 token 的 logprob
            "stream": False,                       # 非流式
            "echo": False
        }
